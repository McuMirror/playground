...
enum DispenserConstants
    ...
    DISPENSER_UPDATE_COUNT     = 110,     /* Copy the CoinCount updated by the application to the dispenser (if possible) */
    DISPENSER_COUNT_UPDATED    = 111,     /* The updated CoinCount has been processed by the dispenser, and the value returned */
    ...
    DISPENSER_RESET_STATUS     = 200      /* This causes Paylink to set the Status field to PAY_FINISHED */
    } ;
...
typedef struct
    {
    int         Unit ;                   /* Specification of this unit */
    int         Status ;                 /* Individual Dispenser Result of last pay command */
...
enum InterfaceNumbers
    ...
    DP_TFLEX_INTERFACE            =  0x000C0000,
    DP_EBDS_INTERFACE             =  0x000D0000,
    DP_DLL_INTERFACE              =  0x000E0000,
    DP_CLS_INTERFACE              =  0x000F0000,
...
enum ManufacturerIdentities
    ...
    DP_MANU_TELEQUIP              =  0x00001000,
    DP_MANU_AES                   =  0x00001100
    } ;
...
enum ManufacturerSpecificDeviceTypes
    ...
    DP_SSP_NOTE                  =   1 | DP_MANU_INNOVATIVE_TECH
                                       | DP_SSP_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

    DP_SSP_RECYCLER              =   2 | DP_MANU_INNOVATIVE_TECH
                                       | DP_SSP_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

                                                    /* Other crane devices */

    DP_TFLEX_TUBE                =   1 | DP_MANU_TELEQUIP
                                       | DP_TFLEX_INTERFACE
                                       | DP_COIN_PAYOUT_DEVICE,


    DP_CX25_TUBE                 =   2 | DP_MANU_TELEQUIP
                                       | DP_TFLEX_INTERFACE
                                       | DP_COIN_PAYOUT_DEVICE,

    DP_CLS                        =  1 | DP_MANU_TELEQUIP
                                       | DP_MANU_TELEQUIP
                                       | DP_COIN_ACCEPT_DEVICE,

    DP_CLS_HOPPER                =   1 | DP_MANU_TELEQUIP
                                       | DP_CLS_INTERFACE
                                       | DP_COIN_PAYOUT_DEVICE,

    DP_MEIBNR                    =   1 | DP_MANU_MARS_ELECTRONICS
                                       | DP_DLL_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

    DP_EBDS_NOTE                 =   1 | DP_MANU_MARS_ELECTRONICS
                                       | DP_EBDS_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

    DP_EBDS_RECYCLER             =   2 | DP_MANU_MARS_ELECTRONICS
                                       | DP_EBDS_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

    DP_EBDS_ROLL                 =   1 | DP_MANU_MARS_ELECTRONICS
                                       | DP_EBDS_INTERFACE
                                       | DP_NOTE_PAYOUT_DEVICE,
    DP_EBDS_BOX                  =       DP_EBDS_ROLL,                        // Historical mis-name

    DP_MARS_SC_ADVANCE           =   3 | DP_MANU_MARS_ELECTRONICS
                                       | DP_EBDS_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

    DP_MARS_SCR_ADVANCE          =   4 | DP_MANU_MARS_ELECTRONICS
                                       | DP_EBDS_INTERFACE
                                       | DP_NOTE_ACCEPT_DEVICE,

    DP_MEIBNR_RECYCLER           =   1 | DP_MANU_MARS_ELECTRONICS
                                       | DP_DLL_INTERFACE
                                       | DP_NOTE_PAYOUT_DEVICE,

    DP_MEIBNR_LOADER             =   2 | DP_MANU_MARS_ELECTRONICS
                                       | DP_DLL_INTERFACE
                                       | DP_NOTE_PAYOUT_DEVICE,
    ...
    DP_CC_CASSETTE                =  1 | DP_MANU_CASHCODE
                                       ...
                                       // Aardvark Special devices

    DP_CASHLESS_HUB               =  1 | DP_MANU_AES
                                       | DP_CCTALK_INTERFACE
                                       | DP_COIN_PAYOUT_DEVICE
    } ;
...
****************************************************************/
void DLL SetPWM (int PWMMaximum);
...
/****************************************************************
Logged I/O API
The new IOBoard maintains a log which continues during the times when
main system power is not present. These interface functions allow access
to that log. Logged switch inputs still form part of the normal Milan
interface accessed by SwitchOpens() and SwitchCloses().
The values returned by these continue to increment for the entire
lifetime of the I/O board battery.

Until Logged Events are marked as processed they will be retained by theIOBoard.
It is necessary for the application to mark events as processed when
they have been, otherwise the buffer space will fill up, and new events
will not be stored.
****************************************************************/
typedef struct
    {
    int     EventTime ;     /* The time (time_t) the event occurred      */
    int     EventType ;     /* The type of the event                     */
    int     EventNumber ;   /* The number of the event (as requested)    */
    int     Information ;   /* Information relevant to the event         */
    } LoggedEventBlock ;

enum ConstantsforLoggedEventType
    {
  /*Name                    Meaning - the time that ...  */
    EVT_POWER_ON,        /* system power was turned on                             */
    EVT_POWER_OFF,       /*   system power was lost                                */
    EVT_QUEUE_OVERFLOW,  /*  the first event was missed due to buffer full         */
    EVT_QUEUE_RECOVERED, /* budffer space became available                         */
    EVT_SYSTEM_RESET,    /*  the system was reset (inplicilty battery failure)     */
    EVT_SWITCH_OPEN,     /* the switch (number in Information) became open         */
    EVT_SWITCH_CLOSED,   /* the switch (number in Information) became closed       */
    EVT_SYSTEM_INIT,     /* The system was initialised (powered and told the time) */
    EVT_NULL             /*  This event should be ignored                          */
    } ;

/****************************************************************
ReadLoggedEvent

The ReadLoggedEvent call retrieves an event as described above from the system.
...
TheEvent
A pointer to an LoggedEventBock object, which is filled in by the call.
...
****************************************************************/
int DLL OldestUnprocessedLoggedEvent (void) ;
...
enum CashlessTypeConstants
    {
    CR_CREDIT_CARD = 0,   // This is a variable credit card based system
    CR_TICKET      = 1    // This is a Fixed credit ticket based system
    } ;



enum CashlessStatusConstants
    {
    CR_NO_UNIT      = 0,    //  No appropriate unit connected
    CR_BUSY         = 1,    //  Busy
    CR_DISABLED     = 2,    //  Idle and disabled
    CR_IDLE         = 3,    //  Idle
    CR_FAULT_DISABLED = 4,  //  The device is non operational while Disabled
    CR_FAULT_IDLE   = 5,    //  The device is non operational while Idle
...
//    These are only valid during Credit Input operations
    ...
    CR_TAKEN        = 13,   //  Credit Actually Taken from remote source
...
typedef struct
    {
    // Static Information
    int   CashlessType ;        //  See CashlessTypeConstants
    char* Description ;         //  Cashless device details
    int   SerialNumber ;        //
    int   TotalAcquisitions ;   //  The lifetime total count of cashless credit acquisition..
    int   TotalCredit ;         //  The lifetime total cashless credit has been acquired.
    // Dynamic Transaction Information
    int   CurrentState ;        //  The state of the system / current transaction - See below
    int   StateDetails ;        //  Extra Details for the current state
    int   CreditValue ;         //  The amount of credit from Paylink for the current status
    char* ReferenceData ;       //  Optional current ticket / card reference number.
    } CashlessControlBlock ;
...
A Paylink system usually includes a sub-system that is responsible
for reporting on and operating real world electrical inputs and outputs.

This control block and access routine enable an interested application
to check on the exact status of this sub-system.
****************************************************************/

typedef struct
    {
    int  DeviceType ;          // See below for the possible contents of this
    int  CurrentStatus ;       // See below for the possible contents of this
    int  NoOfInputs ;          // The number of different inputs supported
    int  NoOfLoggedInputs ;    // The number of inputs whose changes are logged
                               //           (Zero if no log is maintained)
    int  NoOfOutputs ;         // The number of different outputs supported
    int  PWMCapable ;          // Non zero if the device can do PWM
    int  BatteryPerCent ;      // A value between 0 and 100 giving the current backup battery reading
    int  FutureUse[20] ;       // To allow for more fields without affecting old code
    } InterfaceDevice ;


enum InterfaceDeviceType
    {
    INTERFACE_NONE,        //  No information is available.
    INTERFACE_REV3,        //  Original standard "metal box" Paylink.
    INTERFACE_REV4,        //  Current standard "metal box" Paylink (USB powered)
    INTERFACE_LITE,        //  Paylink Lite
    INTERFACE_LITE2,       //  Paylink Lite2
    INTERFACE_DONGLE,      //  uPaylink Dongle
    INTERFACE_RS232        //  RS232 IOBoard
    } ;


enum DeviceCurrentStatus
    {
    DEVICE_NO_DRIVER,    // The API cannot communicate with the driver program
    DEVICE_NEVER_SEEN,   // The current driver has never seen the device
    DEVICE_GOOD,         //  The Device is fully operational
    DEVICE_INITALISING,  //  The Device is good but not yet ready for use
    DEVICE_ERROR,        //  The Device is (has been) present, but is not working
    DEVICE_BUSY          //  The Device is (has been) present, but is not available
    } ;
...
****************************************************************/
typedef enum {
    GENOA_UNKNOWN          = 0,      /* This will be returned by old versions of the firmware */
    GENOA_H8               = 1,      /* The orginal 16 I/O Genoa unit */
    GENOA_RX_HID           = 2,      /* The new USB HUB Genoa unit, using the HID interface */
    GENOA_RX_FTDI          = 3,      /* The new 32 I/O + PWM + Battery Genoa unit, using the same FTDI interface as the H8 */
    GENOA_LITE_2           = 4,      /* The 4 I/O Lite unit */
    GENOA_USB_ONLY         = 5,      /* A driver running with a donlge to drive USB units only */
    GENOA_USB_MERGED       = 6       /* A driver running with a donlge to drive USB units only */
    } PlatformValues ;
/****************************************************************
